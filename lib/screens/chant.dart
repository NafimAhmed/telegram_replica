...
//
//     // 🔧 NEW: if non-text and link missing, build fallback URL when we have msg.id
...
//   });
//
//   // >>> FIXED: normalize msg_id/photo and keep link if provided
...
//       children.add(_ReplyPreview(id: msg.replyTo!, text: rpText, mediaType: rpType));
//     }
//
//     if (mt == 'text') {
//       final ghost = (msg.existsOnTelegram == false) || (msg.status == MessageStatus.pending);
...
//           msg.mediaLink!,
//           fit: BoxFit.cover,
//           gaplessPlayback: true, // reduce flicker when URL updates
...
//                 Colors.red.withOpacity(0.50), // ফ্যাকাশে লাল
//                 BlendMode.modulate,           // ইমেজের সাথে গুণ হয়ে টিন্ট দেয়
//               ),
//               child: img,
//             ),
//           );
...
//   const _ReplyPreview({super.key, required this.id, required this.text, required this.mediaType});
//
//   @override
//   Widget build(BuildContext context) {
...
import 'package:url_launcher/url_launcher.dart';
import 'package:http/http.dart' as http; // ⬅️ NEW: delete POST
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (_currentLang != lang) {
      _currentLang = lang;
      _translateRecent(lang);       // নিচে #6 এ ফাংশন আছে
      setState(() {});
    }
  }
  ...
  void _replySelected() {
    ...
    if (target != null) {
      setState(() {
        _replyToMsgId = target;
        _replyPreviewText = _byId[target]?.previewText();
      });
      _exitSelection();
  ...
  void _startTypingSweeper() {
    ...
    _typingTtlSweeper = Timer.periodic(const Duration(seconds: 2), (_) {
      final now = DateTime.now().toUtc();
      final expired = _typingTtl.entries
  ...
  void _handleFrameString(String s) {
    ...
    switch (action) {
      ...
      case 'typing':
        {
          final sender =
          (m['sender_id'] is int) ? m['sender_id'] as int : null;
          if (sender != null) {
            _typingUserIds.add(sender);
            _typingTtl[sender] =
  ...
  void _insertOrUpdate(ChatMessage msg) {
    ...
    // 🔧 if non-text and link missing, build fallback URL when we have msg.id
    if ((msg.mediaType ?? 'text') != 'text' &&
        (msg.mediaLink == null || msg.mediaLink!.isEmpty) &&
        msg.id != null) {
      msg.mediaLink = _buildMediaUrl(msg.id!);
  ...
  void _scrollAfterIncoming() {
    if (!_scrollCtrl.hasClients) return;
    final delta = _scrollCtrl.position.maxScrollExtent -
  ...
  Future<void> _deleteSelected() async {
    ...
    if (ok != true) return;
    if (!mounted) return; // dialog বন্ধ হওয়ার পর screen dispose হয়ে গেলে
    ...
    try {
      ...
      if (!mounted) return; // নেটওয়ার্ক রাউন্ড-ট্রিপের মধ্যে স্ক্রিন dispose হলে

      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        final deleted = <int>{};
        try {
          final m = json.decode(resp.body);
          final cand = (m['deleted'] ?? m['deleted_ids'] ?? m['data']) as dynamic;
          if (cand is List) {
            for (final e in cand) {
        ...
        if (!mounted) return;
        setState(() {});                 // 🔒 mounted guard উপরে আছে
        _showSnack('Deleted ${effective.length} message(s).');

        if (!mounted) return;
        _exitSelection();                // selection clear
      } else {
...
class _MessageBubble extends StatelessWidget {
  ...
  Widget _buildBody(BuildContext context) {
    ...
    if (mt == 'text') {
      ...
      //     style: TextStyle(
      //       fontStyle:
      //       (msg.existsOnTelegram == false) ? FontStyle.italic : FontStyle.normal,
      //       color: (msg.existsOnTelegram == false) ? Colors.black54 : null,
      //     ),
      //   ));
      // }
