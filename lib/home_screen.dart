...
import 'package:internet_connection_checker/internet_connection_checker.dart'; // ✅ ADD
import '../url.dart';
...
class _TelegraphHomeState extends State<TelegraphHome>
  ...
  // 🔌 Internet status + persistent snackbar
  bool _hasInternet = true;
  StreamSubscription<ConnectivityResult>? _connSub;
  StreamSubscription<InternetConnectionStatus>? _pingSub;

  bool _noNetSnackVisible = false; // snackbar flag
  late final SnackBar _offlineSnack = const SnackBar(
    content: Text('⚠️ No Internet Connection'),
    duration: Duration(days: 1), // নেট না আসা পর্যন্ত থাকবে
  ...
  Future<void> _initNetWatch() async {
    _hasInternet = await InternetConnectionChecker().hasConnection;
    if (mounted) {
      setState(() {});
      if (!_hasInternet) {
        // প্রথম ফ্রেমের পর দেখাই যাতে ScaffoldMessenger ready থাকে
        WidgetsBinding.instance.addPostFrameCallback((_) => _showNoInternetSnack());
      }
    }

    _connSub = Connectivity().onConnectivityChanged.listen((_) async {
      final real = await InternetConnectionChecker().hasConnection;
      if (!mounted) return;
      setState(() => _hasInternet = real);
      if (!real) {
        _showNoInternetSnack();
      } else {
        _hideNoInternetSnack();
      }
    }) as StreamSubscription<ConnectivityResult>?;

    _pingSub = InternetConnectionChecker()
        .onStatusChange
        .listen((InternetConnectionStatus s) {
      if (!mounted) return;
      final real = s == InternetConnectionStatus.connected;
      setState(() => _hasInternet = real);
      if (!real) {
        _showNoInternetSnack();
      } else {
  ...
  void _showNoInternetSnack() {
    if (!mounted) return;
    if (_noNetSnackVisible) return; // already showing
    _noNetSnackVisible = true;
    ScaffoldMessenger.of(context).showSnackBar(_offlineSnack).closed.then((_) {
      // ইউজার swipe করে বন্ধ করলে—flag reset; পরের ইভেন্টে আবার দেখাবে
      _noNetSnackVisible = false;
    });
  }

  void _hideNoInternetSnack() {
    if (!mounted) return;
    if (_noNetSnackVisible) {
      _noNetSnackVisible = false;
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
    }
  }

  // 🔙 Back প্রেসে Exit confirm
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope( // ✅ ADD
      onWillPop: _confirmExit, // ✅ ADD
      child: Scaffold(
        drawer: const TelegraphDrawer(),
        appBar: AppBar(
          title: Text(p.firstName.isNotEmpty ? p.firstName : "Telegraph"),
          actions: [
            IconButton(
  ...
  Widget _buildSettingsTab() => ListView(
    children: const [
      ...
      ListTile(leading: Icon(Icons.color_lens_outlined), title: Text("Theme")),
    ],
  );
}
...
class _TelegraphDrawerState extends State<TelegraphDrawer> {
  bool showAccounts = false;
  bool _isSwitching = false; // 🔹 দ্রুত switch indicator
          ...
          Container(
            ...
            child: Column(
              children: [
                    ...
                    items: langs
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Text(lang['name']!, style: const TextStyle(color: Colors.black)),
                        ],
                      ),
                    ))
                        .toList(),
                ...
                Expanded(
                  child: ListView(children:  [
                    const ListTile(leading: Icon(Icons.person), title: Text("My Profile")),
                    const ListTile(leading: Icon(Icons.star_border), title: Text("Special Features")),
                    ListTile(
                      leading: const Icon(Icons.chat_bubble_outline),
                      title: const Text("New Group"),
                      onTap: () {
                    ...
                    const Divider(),
                    const ListTile(leading: Icon(Icons.contacts), title: Text("Contacts")),
                    const ListTile(leading: Icon(Icons.call), title: Text("Calls")),
                    const ListTile(leading: Icon(Icons.settings), title: Text("Settings")),
                    const ListTile(leading: Icon(Icons.color_lens_outlined), title: Text("Theme Settings")),
                    const ListTile(leading: Icon(Icons.group_add_outlined), title: Text("Invite Friends")),
                    const Divider(),
                    const ListTile(
                        leading: Icon(Icons.logout, color: Colors.red),
                        title: Text("Logout All", style: TextStyle(color: Colors.red))),
                  ]),
                ),
              ],
            ),
